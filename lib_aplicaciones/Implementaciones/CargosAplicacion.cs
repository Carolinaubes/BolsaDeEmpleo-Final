...
namespace lib_aplicaciones.Implementaciones
{
    public class CargosAplicacion : ICargosAplicacion
    {
        ...
        public CargosAplicacion(ICargosRepositorio iRepositorio)
        {
        ...
        public List<Cargos> Buscar(Cargos entidad, string tipo)
        {
            Expression<Func<Cargos, bool>>? condiciones = null;
            switch (tipo.ToUpper())
            {
                case "NOMBRE": condiciones = x => x.Nombre!.Contains(entidad.Nombre!); break;
                case "CODIGO ESTUDIO": condiciones = x => x.Cod_estudio == entidad.Cod_estudio; break;
                case "COMPLEJA":
                    condiciones =
                        x => x.Nombre!.Contains(entidad.Nombre!) ||
                             x.Cod_estudio == entidad.Cod_estudio; break;
